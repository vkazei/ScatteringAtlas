...
\ddplot{SHSH/SHSHCij}{SHSH/SHSHnPar0}{Same as \figref{PP_Full/PPCij_PP_Full/PPnPar0}, but for $SH-SH$ scattering. Scattering on density depends only on the angle between polarization vectors of incident and scattered wavefields, for $SH$ reflections from a horizontal reflector these vectors are parallel, hence the pattern is constant in $C_{ij}, \rho$ parameterization. $SH$ scattering is only sensitive to monoclinic parameters, and, thus, is non-sensitive to the perturbations of $C_{.3}$ parameters, $V_p$, $\epsilon_1$, $\eta_1$, and $\eta_d$. $C_{11}$ is coupled to both $C_{22}$ and $C_{12}$. $C_{16}$ is coupled to $C_{26}$.}
